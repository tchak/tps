  ...
  class ExportPreloader
    ...
    def in_batches
      ...
      (dossiers.size.to_f / EXPORT_BATCH_SIZE).ceil.times do |i|
        ...
        load_dossiers(dossiers[start_index..end_index])
      end
      dossiers
    end
    ...
    def positions
      @positions ||= ProcedureRevisionTypeDeChamp
        .where(revision_id: @dossiers.distinct(:revision_id).pluck(:revision_id))
    ...
    def load_dossiers(dossiers)
      ...
      champs_by_parent = children_champs.group_by(&:parent_id)
      dossiers.each do |dossier|
        load_dossier(dossier, champs_by_dossier[dossier.id], champs_by_parent)
      end
    end
    ...
    def load_dossier(dossier, champs, children_by_parent = {})
      ...
      load_champs(dossier, :champs, champs_public)
      load_champs(dossier, :champs_private, champs_private)
      ...
      champs.filter(&:repetition?).each do |parent_champ|
        ...
        load_champs(dossier, :champs, champs, parent_champ)
    ...
    def load_champs(dossier, name, champs, parent = nil)
      champs.each do |champ|
        champ.association(:dossier).target = dossier
      end

      (parent || dossier).association(name).target = champs.sort_by do |champ|
        positions.fetch(dossier.revision_id, {})[champ.type_de_champ_id]
      end
    end
  end
